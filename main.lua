	...
	elseif arg1 == 'RightButton' and button.flyoutParent then
		...
		if oldAction ~= newAction then
			...
			if as and bs then
				...
				-- TODO 宏图标仅支持索引，为字符串无效 xhwsd@qq.com 2025-10-21
...
function Flyout_Hide(keepOpenIfSticky)
	...
	if getglobal('FlyoutButton1') and not getglobal('FlyoutButton1'):IsVisible() and getglobal('FlyoutButton1').flyoutParent then
		local arrow = getglobal(getglobal('FlyoutButton1').flyoutParent:GetName() .. 'FlyoutArrow')
...
function Flyout_Show(button)
	...
	for index, action in button.flyoutActions do
		...
		if texture then
			...
			item:SetWidth(size)
			item:SetHeight(size)
			item.cooldown:SetScale(size / item.cooldown:GetWidth())  -- Scale cooldown so it will stay centered on the button.
			...
			item:GetPushedTexture():SetTexture(texture)  -- Without this, icons disappear on click.
